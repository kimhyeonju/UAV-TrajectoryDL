...
from GRU_model_drone import create_gru_model
...
# 결과 저장 경로 설정
predictions_path = '/Users/admin/PycharmProjects/GP_test/GRU_model/test_predictions_drone_newModel/'
plots_path = '/Users/admin/PycharmProjects/GP_test/GRU_model/test_plot_drone_newModel/'
...
for file_name in file_list:
    ...
    # 예측 결과를 CSV 파일로 저장
    predicted_df.to_csv(f'{predictions_path}{file_name}_test_predictions_newModel.csv', index=False)
    ...
    # 축 범위 설정 및 간격 조정
    longitude_range = [min(actual_df['Longitude'].min(), predicted_df['Longitude'].min()) - 1,
                       max(actual_df['Longitude'].max(), predicted_df['Longitude'].max()) + 1]
    latitude_range = [min(actual_df['Latitude'].min(), predicted_df['Latitude'].min()) - 1,
                      max(actual_df['Latitude'].max(), predicted_df['Latitude'].max()) + 1]
    altitude_range = [min(actual_df['Altitude (m)'].min(), predicted_df['Altitude (m)'].min()) - 1000,
                      max(actual_df['Altitude (m)'].max(), predicted_df['Altitude (m)'].max()) + 1000]
...
# 최종 학습된 모델 저장
model.save('/Users/admin/PycharmProjects/GP_test/GRU_model/GRU_model_drone_layer.h5')