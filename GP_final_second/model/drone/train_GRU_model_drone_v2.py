...
from GRU_model_v3 import create_weighted_input_model
...
def create_dataset(data, look_back, forward_length):
    ...
    for i in range(len(data) - look_back - forward_length):
        X.append(data[i:(i + look_back), :])  # 입력 파라미터: 6개 (위도, 경도, 고도, 속도, 풍속, 풍향)
...
# 결과 저장 경로 설정
predictions_path = '/Users/admin/PycharmProjects/GP_test/GP_final_second/data/drone/GRU_model_v4/look_back=10&forward=0/test_predictions_general/'
plots_path = '/Users/admin/PycharmProjects/GP_test/GP_final_second/data/drone/GRU_model_v4/look_back=10&forward=0/test_plot_general/'
...
# 표준 출력을 파일로 리디렉션
log_file_path = '/Users/admin/PycharmProjects/GP_test/GP_final_second/data/drone/GRU_model_v4/look_back=10&forward=0/training_log_general.txt'
...
for file_name in file_list:
    ...
    # 데이터 스케일링
    df_filtered.loc[:, ['Latitude', 'Longitude', 'Altitude (m)', 'Speed (m/s)', 'wind_speed', 'wind_direction']] = scaler.fit_transform(df_filtered[['Latitude', 'Longitude', 'Altitude (m)', 'Speed (m/s)', 'wind_speed', 'wind_direction']])
    ...
    model = create_weighted_input_model(
        input_shape=(look_back, 6))  # 입력: 6개의 입력 특성 (Latitude, Longitude, Altitude (m), Speed, wind_speed, wind_direction)
    ...
    predicted_coordinates = scaler.inverse_transform(np.concatenate([test_predictions, np.zeros((test_predictions.shape[0], 3))], axis=1))

    # 테스트 데이터의 실제 지점을 원래 스케일로 변환 (위도, 경도, 고도만)
    actual_coordinates = scaler.inverse_transform(np.concatenate([y_test, np.zeros((y_test.shape[0], 3))], axis=1))
...
# 최종 학습된 모델 저장
model.save(
    '/Users/admin/PycharmProjects/GP_test/GP_final_second/SaveModel/drone_SaveModel/GRU_model_v4/look_back=10&forward=0/gru_model_4.h5')
