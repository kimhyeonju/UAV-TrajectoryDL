...
def create_dataset(data, look_back, forward_length):
    ...
    for i in range(len(data) - look_back - forward_length):
        ...
        y.append(data[i + look_back + forward_length,
                 :3])  # 6개의 출력 (Latitude, Longitude, Altitude, Speed, wind_speed, wind_direction)
...
for experiment in experiment_settings:
    ...
    for file_name in file_list:
        ...
        df_filtered.loc[:, ['Latitude', 'Longitude', 'Altitude (m)', 'Speed (m/s)', 'wind_speed',
                            'wind_direction']] = scaler.fit_transform(
        ...
        # 모델 생성 및 학습
        model = create_model(model_type=experiment['model_type'], input_shape=(look_back, 6),
