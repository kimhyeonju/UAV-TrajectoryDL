...
# 결과 저장 경로 설정
predictions_path = '/Users/admin/PycharmProjects/GP_test/GP_final/data/aircraft/test_predictions_extreme/'
plots_path = '/Users/admin/PycharmProjects/GP_test/GP_final/data/aircraft/test_plot_extreme/'
...
# 표준 출력을 파일로 리디렉션
log_file_path = '/Users/admin/PycharmProjects/GP_test/GP_final/data/aircraft/training_log_extreme.txt'
...
for file_name in file_list:
    ...
    df_filtered = df[['Latitude', 'Longitude', 'Altitude (m)']].copy()  # 복사본 생성

    # 데이터 스케일링
    df_filtered[['Latitude', 'Longitude', 'Altitude (m)']] = scaler.fit_transform(df_filtered[['Latitude', 'Longitude', 'Altitude (m)']])
    ...
    # 모델 생성 및 학습
    model = Sequential()
    model.add(Input(shape=(look_back, 3)))  # Input 객체 사용
    ...
    model.fit(X_train, y_train, epochs=200, batch_size=16, validation_split=0.2, callbacks=[early_stopping, reduce_lr], verbose=0)
    ...
    # 예측 결과를 CSV 파일로 저장
    predicted_df.to_csv(f'{predictions_path}{file_name}_test_predictions_extreme.csv', index=False)
...
# 최종 학습된 모델 저장
model.save('/Users/admin/PycharmProjects/GP_test/GP_final/aircraft/model/GRU_model_extreme.h5')
